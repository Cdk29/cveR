...
One fonction as wrapper. For some reason the more elaborate plot cannot be operated in the app. "no token object".
...
topic_model_plot <- topic_model_BTM_plot_generation(test_df, ud_model_file)
```
...
The build of this function fail for whatever reason when using fusen. So I created manually.

```{r}
#' @title Topic Modeling with Cooccurrence
#' @description This function applies the Biterm Topic Model (BTM) to a given annotated dataframe,
...
btm_topic_model_cooccurrence <- function(df, k = 10, iter = 2000) {
  ...
  head(df,1)
  head(biterms,1)
  biterms <- biterms[, udpipe::cooccurrence(x = token,
...
test_df <- tail(df, 600)
ud_model_file <- "english-ewt-ud-2.5-191206.udpipe"
topic_model_plot <- topic_model_btm(test_df, ud_model_file)
```
